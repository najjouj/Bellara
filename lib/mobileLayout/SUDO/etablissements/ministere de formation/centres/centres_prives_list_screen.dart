...
class CentresPrivesListScreen extends StatefulWidget {
  final String governorat;
  const CentresPrivesListScreen({super.key, required this.governorat});

  @override
  State<CentresPrivesListScreen> createState() =>
      _CentresPrivesListScreenState();
}

class _CentresPrivesListScreenState extends State<CentresPrivesListScreen> {
    ...
    if (widget.governorat == '')
      ...
      return StreamBuilder(
        stream: etabDBoperations()
            ...
            .collection('centres de formations')
            .where('type', isEqualTo: 'privé')
            .where('gouvernorat', isEqualTo: widget.governorat)
            .snapshots(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(
